...
void EnemyComponent::Initialize() {
	...
	BahamutAI::RegisterOwnerAction<EnemyComponent>(
		...
		{ 
			...
			owner.owner_->GetTransform().translation_.x += direction.x * speed; 
			owner.owner_->GetTransform().translation_.y += direction.y * speed; 
			owner.owner_->GetTransform().translation_.z += direction.z * speed; 
...
BahamutAI::BTStatus EnemyComponent::MoveToTarget(BahamutAI::AIContext& context) {
	...
	float dx = targetTransform.translation_.x - self.translation_.x;
	float dy = targetTransform.translation_.y - self.translation_.y;
	float dz = targetTransform.translation_.z - self.translation_.z;
	...
	self.translation_.x += dx / distance * speed_;
	self.translation_.y += dy / distance * speed_;
	self.translation_.z += dz / distance * speed_;
