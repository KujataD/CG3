...
public:
	...
	/// <summary>
	/// モデル階層を描画し、drawEditorOverlaysならグリッド/アイコン/フラスタム等を足す(ワールド行列更新はしない)。
	/// 1ビューぶんの描画実体。ISceneRenderer実装(SampleScene)からビュー毎に呼ばれる。非virtual=ABI非破壊。
	/// </summary>
	KUJAKU_API void DrawSceneContents(bool drawEditorOverlays);
