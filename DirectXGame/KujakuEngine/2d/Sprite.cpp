...
namespace KujakuEngine {
	...
	Sprite* Sprite::Create(uint32_t index, const Vector2& position, float width, float height, const Vector4& color, Vector2 anchorPoint) {
		Sprite* sprite = new Sprite();
		sprite->position_ = position;
		sprite->size_ = { 1.0f, 1.0f };

		sprite->CreateVertexBuffer(width, height, anchorPoint);
	...
	void Sprite::SetVertexMap(float width, float height, Vector2 anchorPoint) {
		vertexMap_[0].position = { -width * anchorPoint.x, height * (1.0f - anchorPoint.y), 0.0f, 1.0f }; // 左下
		vertexMap_[0].texcoord = { 0.0f, 1.0f };
		vertexMap_[0].normal = { 0.0f, 0.0f, -1.0f };

		vertexMap_[1].position = { -width * anchorPoint.x, -height * anchorPoint.y, 0.0f, 1.0f }; // 左上
		vertexMap_[1].texcoord = { 0.0f, 0.0f };
		vertexMap_[1].normal = { 0.0f, 0.0f, -1.0f };

		vertexMap_[2].position = { width * (1.0f - anchorPoint.x), height * (1.0f - anchorPoint.y), 0.0f, 1.0f }; // 右下
		vertexMap_[2].texcoord = { 1.0f, 1.0f };
		vertexMap_[2].normal = { 0.0f, 0.0f, -1.0f };

		vertexMap_[3].position = { width * (1.0f - anchorPoint.x), -height * anchorPoint.y, 0.0f, 1.0f }; // 右上
		vertexMap_[3].texcoord = { 1.0f, 0.0f };
		vertexMap_[3].normal = { 0.0f, 0.0f, -1.0f };
	}

	void Sprite::CreateVertexBuffer(float width, float height, Vector2 anchorPoint) {
		...
		SetVertexMap(width, height, anchorPoint);
	}
