...
public:
	...
	/// </summary>
	static KUJATA_API Model* CreateFromOBJ(const std::string& objname, ShaderModel shaderModel = ShaderModel::kNone);
	static KUJATA_API Model* CreateFromGlTF(const std::string& objname, ShaderModel shaderModel = ShaderModel::kNone);
	static KUJATA_API Model* TryCreateFromFile(const std::string& filePath, ShaderModel shaderModel = ShaderModel::kNone);

	static KUJATA_API Model* CreateSphere(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone, uint32_t subdivision = 16);

	static KUJATA_API Model* CreateCube(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone);

	/// <summary>
	/// カプセル(cylinder + 両端半球)のテンプレモデルを生成する。
	/// heightは両端の半球を含む全長で、cylinder部分の長さは height - 2*radius。
	/// </summary>
	static KUJATA_API Model* CreateCapsule(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone, float radius = 0.5f, float height = 2.0f, uint32_t subdivision = 16);
	...
	/// </summary>
	KUJATA_API void DrawShadow(const WorldTransform& worldTransform, const Matrix4x4& lightViewProjection);

	static KUJATA_API Model* CreatePlane(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone);

	static KUJATA_API Model* CreateTriangle(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone);

	static KUJATA_API Model* CreateTetrahedron(const std::string& textureFilePath, ShaderModel shaderModel = ShaderModel::kNone);
	...
	/// </summary>
	static KUJATA_API void PreDraw();

	/// <summary>
	/// 描画後処理（将来の拡張のために用意）
	/// </summary>
	static KUJATA_API void PostDraw();

	/// <summary>
	/// 描画（PreDraw の後に呼ぶ）
	/// </summary>
	KUJATA_API void Draw(const WorldTransform& worldTransform, const Camera& camera, FillMode fillMode = kFillModeSolid);
	...
	KUJATA_API void SetUVTransform(const Vector2& offset, const Vector2& scale, float rotation);

	// サブメッシュ単位のUVトランスフォーム(MultiMeshのパーツ別UV調整用。indexが範囲外なら無視)。
	KUJATA_API void SetSubMeshUVTransform(size_t index, const Vector2& offset, const Vector2& scale, float rotation);
